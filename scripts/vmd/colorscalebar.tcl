...
proc ::ColorScaleBar::color_scale_bar {{length 0.5} {width 0.05} {auto_scale 1} {fixed 1} {min 0} {max 100} {label_num 5} {text 16} {fp_format 0} {x_pos -1.0} {y_pos -1.0} {replacebar 1} {molid top} {repid 0} {showlegend 0} {legend "Color Scale:"}} {
  ...
  for {set i 4} {$i <= $label_num } { incr i 5} {
     draw color $text
   ...
   set labeltxt ""
    if { $fp_format == 0 } {
      ...
      set labeltxt [format "% #.0f "  $cur_text]
    set labeltxt [format " %d" [expr $i + 1]]
      draw text  "$coord_x $coord_y $use_z" "$labeltxt" size 1 thickness 3
   }
    #draw text  "$coord_x $coord_y $use_z" "$labeltxt" size 1 thickness 3
    ...
    #
    draw color $text
    set coord_y [expr $start_y]
    set cur_text [expr $min ]
    set labeltxt [format " %d" [expr 0 + 1]]
    draw text  "$coord_x $coord_y $use_z" "$labeltxt" size 1 thickness 3
    draw line "[expr $use_x+$width] $coord_y $use_z" "[expr $use_x+(1.45*$width)] $coord_y $use_z" width 2
    ...
    #
    draw color $text
    set coord_y [expr $start_y + $label_num * $step_size ]
    set cur_text [expr $min + $i * $value_step ]
    set labeltxt [format " %d" [expr $label_num + 1]]
    draw text  "$coord_x $coord_y $use_z" "$labeltxt" size 1 thickness 3
    draw line "[expr $use_x+$width] $coord_y $use_z" "[expr $use_x+(1.45*$width)] $coord_y $use_z" width 2
...
proc ::ColorScaleBar::gui { } {
  ...
  frame $w.labelformat
  label $w.labelformat.label -text "Display text: "
  radiobutton $w.labelformat.decimal -text "Off" -value "0" \
    -variable ::ColorScaleBar::fpformat
  radiobutton $w.labelformat.scientific -text "On" -value "1" \
    -variable ::ColorScaleBar::fpformat
